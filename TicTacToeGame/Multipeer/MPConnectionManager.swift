...
class MPConnectionManager: NSObject, ObservableObject {
    ...
    var game: GameServise?
    
    func setup(game: GameServise) {
        self.game = game
    }
    
    @Published var availablePeers = [MCPeerID]()
    @Published var recieveInvites: Bool = false
    @Published var recieveInviteForm: MCPeerID?
...
extension MPConnectionManager: MCNearbyServiceAdvertiserDelegate {
    func advertiser(_ advertiser: MCNearbyServiceAdvertiser, didReceiveInvitationFromPeer peerID: MCPeerID, withContext context: Data?, invitationHandler: @escaping (Bool, MCSession?) -> Void) {
        DispatchQueue.main.async {
            self.recieveInvites = true
            self.recieveInviteForm = peerID
